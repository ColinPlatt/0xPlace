...
import {LibString} from "./ExtLibString.sol";
...
function transactionObject(Object memory _object) pure returns (Transaction memory) {
    return Transaction(
        string.concat(
            '{"from": "',
...
function transactionObjectByTag(ObjectTags memory _object) pure returns (Transaction memory) {
    ...
    unchecked {
        for (uint256 i = 0; i < _object.tags.length; i++) {
